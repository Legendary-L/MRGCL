...
def restartable_random_walk_adjacency(adj, walk_length, restart_prob=0.1):
    ...
    for node in range(num_nodes):
        ...
        for _ in range(walk_length):
            if random.random() < restart_prob:
                current_node = node
            neighbors = torch.nonzero(adj[current_node]).flatten()
            if neighbors.numel() > 0:
                next_node = random.choice(neighbors)
                new_adj[current_node][next_node] = 1.0
...
class MRCGNN(nn.Module):
    ...
    def forward(self, data_o, data_s, data_a, idx):
        ...
        # 假设您已经有了边索引 edge_index 和想要的游走长度 walk_length
        # 例如 edge_index = torch.tensor([[0, 1, 1, 2, 3], [1, 0, 2, 1, 2]])
        # new_adj = restartable_random_walk_adjacency(edge_index, walk_length)
        x_a = restartable_random_walk_adjacency(x_a, 2)
