...
def read_first_column(file_path):
    first_column_data = set()
    with open(file_path, 'r') as file:
        for line in file:
            data = line.strip().split('\t')[0]  # 假设数据以制表符分隔，并且第一列是要提取的数据
            first_column_data.add(data)
