...
def train_model(model, optimizer, data_o, data_s, data_a, train_loader, val_loader, test_loader, args):
    ...
    for epoch in range(args.epochs):
        ...
        # 计算MRR
        #mrr = MRR(y_label_train, y_pred_train1)
    
        # 计算Hits@3 和 Hits@10
        #hits_at_3 = HitsAtN(y_label_train, y_pred_train1, 3)
        #hits_at_10 = HitsAtN(y_label_train, y_pred_train1, 10)
...
#
        if not args.fastmode:
            ...
            acc_val, f1_val, recall_val,precision_val, loss_val, MRR_val, Hits3_val, Hits10_val = test(model, val_loader, data_o, data_s, data_a, args,0)
            ...
            print('epoch: {:04d}'.format(epoch + 1),
                  ...
                  #'Hits@3_val: {:.4f}'.format(Hits3_val),
                  #'Hits@10_val: {:.4f}'.format(Hits10_val),
...
def test(model, loader, data_o, data_s, data_a, args,printfou):
   ...
   # 计算MRR
    #mrr = 0
    #for i in range(y_pred_train.shape[0]):
    #    rank = np.argsort(y_pred_train[i])[::-1].tolist().index(y_label_train[i])
    #    mrr += 1 / (rank + 1)
    #mrr /= y_pred_train.shape[0]

    # 计算Hits@3和Hits@10
    #hits_at_3 = np.mean([1 if y_label_train[i] in np.argsort(-y_pred_train[i])[:3] else 0 for i in range(y_pred_train.shape[0])])
    #hits_at_10 = np.mean([1 if y_label_train[i] in np.argsort(-y_pred_train[i])[:10] else 0 for i in range(y_pred_train.shape[0])])
    ...
    #return acc,f1_score1,recall1,precision1,loss, mrr, hits_at_3, hits_at_10
